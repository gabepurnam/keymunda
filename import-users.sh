...
kc_lookup_username() {
  ...
  userid=`echo $result | grep -Eo '"id":.*?[^\\]"' | cut -d','  -f 1 | cut -d':'  -f 2 | sed -e 's/"//g'`
...
unit_test() {
  ...
  kc_login
  kc_create_user johnwest "John" "Joe West" john@example.com
  kc_set_pwd $userid ":Frepsip4"
  kc_set_group_hard $userid 40e4ff01-ba9a-4581-a314-d05c63ff3a52
...
# CSV file format: "first name, last name, username, email, password, groupid"
# P.S.: Modified to import 100 users per login due to expiration time
import_accts() {
  kc_login
  ...
  while read -r line; do
    ...
    if [ $counter -lt 100 ]; then
      ...
      if [ -z "$userid" ]; then # if user id do not exists, insert
        kc_create_user "${arr[0]}" "${arr[1]}" "${arr[2]}" "${arr[3]}"

        #kc_update_user "$userid" "${arr[0]}" "${arr[1]}" "${arr[2]}"
...
#        [ $? -ne 0 ] || kc_set_group_hard "$userid" "${arr[5]}" #skip if kc_create_user failed
        kc_set_group_hard "$userid" "${arr[5]}" #skip if kc_create_user failed
      else
        echo $userid
        echo $line >> "$csv_file".imported
      fi
    else
      #awk -F',' 'FNR==NR{a[$1];next};!($1 in a)' examples.csv.imported example.csv > example.missing.csv
      kc_login
      counter=0
    fi
